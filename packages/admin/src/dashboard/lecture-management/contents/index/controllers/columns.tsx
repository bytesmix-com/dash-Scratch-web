...
export const columns = [
  ...
  },
  {
    ...
    accessor: "files",
    Cell: ({ ...props }: CellProps<VideoModel>) => {
      const { files } = props.row.original;

      if (_.isEmpty(files)) return <Text>-</Text>;
  ...
  },
  {
    ...
    accessor: "isPublic",
    Cell: ({ ...props }: CellProps<VideoModel>) => {
      const { playlist, isPublic } = props.row.original;
      const isRecommended = !playlist;
      const publicVid = isPublic || isRecommended;
      return (
        <HStack spacing={0.5}>
          <SvgIcon
            name={publicVid ? "icon/public" : "icon/private"}
            boxSize="14px"
          />
          <Text variant="body-3">{publicVid ? "공개" : "비공개"}</Text>
        </HStack>
      );
    },
  },
  {
    ...
    Cell: ({ ...props }: CellProps<VideoModel>) => {
      const { id, playlist } = props.row.original;
